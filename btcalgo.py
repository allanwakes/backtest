import backtrader as bt
import datetime as dt
from db import create_order

leverUp = False


''' Buy & Hold Bitcoin '''


class BitcoinFeed(bt.feeds.GenericCSVData):
    params = (('openinterest', -1),)


class BitcoinStrategy(bt.Strategy):

    def __init__(self):
        self.BTC = self.datas[0]
        self.smaShort = bt.ind.SimpleMovingAverage(self.BTC.close, period=100)
        self.smaLong = bt.ind.SimpleMovingAverage(self.BTC.close, period=400)
        self.vol = bt.ind.SimpleMovingAverage(self.BTC.volume, period=80)
        self.last_id = 0
        super().__init__()

    def get_id(self):
        self.last_id += 1
        return self.last_id

    def log(self, txt, dt=None):
        dt = dt or self.BTC.datetime.datetime()
        print('%s %s' % (dt.isoformat(), txt))

    def do_exit(self, asset, short=False):
        if self.getposition(asset).size != 0.0:
            trade_id = self.get_id()
            if short:
                self.buy(asset, tradeid=trade_id, exectype=bt.Order.Market)
            else:
                self.sell(asset, tradeid=trade_id, exectype=bt.Order.Market)
            self.log("Order %s Exit %s %s %.3f" % (trade_id, "Short" if short else "Long", asset._name, self.BTC.close[0]))

    def do_enter(self, asset, short=False):
        if self.getposition(asset).size == 0.0:
            trade_id = self.get_id()
            if short:
                self.sell(asset, tradeid=trade_id, exectype=bt.Order.Market)
            else:
                self.buy(asset, tradeid=trade_id, exectype=bt.Order.Market)
            self.log("Order %s Enter %s %s %.3f" % (trade_id, "Short" if short else "Long", asset._name, self.BTC.close[0]))

    def notify_order(self, order):
        if order.status in [bt.Order.Submitted, bt.Order.Accepted]:
            return  # Await further notifications

        self.log("Order %s %s Size %.2f Price: %.3f, Cost: %.2f, Comm %.2f" % (
        order.tradeid, order.Status[order.status], order.size, order.executed.price, order.executed.value,
        order.executed.comm))
        create_order(
            price=order.executed.price, amount=abs(order.size),
            commission=order.executed.comm, order_type='buy' if order.size > 0 else 'sell',
            date=self.BTC.datetime.datetime().isoformat()
        )

    def notify_trade(self, trade):
        if not trade.isclosed:
            return

        self.log('OPERATION PROFIT, GROSS %.2f, NET %.2f' %
                 (trade.pnl, trade.pnlcomm))

    def next(self):
        if len(self.broker.get_orders_open()) == 0.0:
            if self.smaLong[0] > self.smaShort[0] and self.vol[0] < self.BTC.volume[0]:
                self.do_exit(self.BTC)
            elif self.smaLong[0] < self.smaShort[0] and self.vol[0] < self.BTC.volume[0]:
                self.do_enter(self.BTC)


if __name__ == "__main__":

    leverage = 2 if leverUp else 1
    percents = 195 if leverUp else 95

    cerebro = bt.Cerebro()

    data = BitcoinFeed(
        name="BTC",
        dataname="krakenUSD_15m.csv",
        timeframe=bt.TimeFrame.Minutes,
        fromdate=dt.datetime(2017, 1, 1),
        todate=dt.datetime(2018, 1, 1),
        nullvalue=0.0
    )

    cerebro.adddata(data)

    cerebro.addstrategy(BitcoinStrategy)
    cerebro.addsizer(bt.sizers.PercentSizer, percents=percents)  # taking leverage

    broker = cerebro.getbroker()
    broker.setcash(1000)

    broker.setcommission(commission=0.003, leverage=leverage, name="BTC")  # allowing leverage
    broker.set_filler(bt.broker.filler.FixedBarPerc(perc=25))


    print("Start value %.4f" % broker.getvalue())

    cerebro.run()

    print("End value %.4f" % broker.getvalue())

    figure = cerebro.plot(style='candlebars')[0][0]
    figure.savefig('example.png')
